...
// By default we talk to the "regular" HelloWorldTest plugin
var helloworldPluginBackend = "janus.plugin.helloworld_plugin";
...
// 	helloworldPluginBackend = "janus.plugin.echolua";
// else if(getQueryStringValue("plugin") === "duktape")
// 	helloworldPluginBackend = "janus.plugin.echojs";
			...
			janus = new Janus(
				{
					...
					success: function() {
						...
						janus.attach(
							{
								plugin: helloworldPluginBackend,
								opaqueId: opaqueId,
								success: function(pluginHandle) {
									$('#details').remove();
									helloworldtest = pluginHandle;
									Janus.log("Plugin attached! (" + helloworldtest.getPlugin() + ", id=" + helloworldtest.getId() + ")");
									// Negotiate WebRTC
									...
									helloworldtest.send({ message: body });
									Janus.debug("Trying a createOffer too (audio/video sendrecv)");
									helloworldtest.createOffer(
										{
											...
											success: function(jsep) {
												...
												helloworldtest.send({ message: body, jsep: jsep });
								...
								onmessage: function(msg, jsep) {
									...
									if(jsep) {
										...
										helloworldtest.handleRemoteJsep({ jsep: jsep });
									}
								...
								onlocaltrack: function(track, on) {
									...
									if(helloworldtest.webrtcStuff.pc.iceConnectionState !== "completed" &&
											helloworldtest.webrtcStuff.pc.iceConnectionState !== "connected") {
								...
								onremotetrack: function(track, mid, on, metadata) {
									...
									if(track.kind === "audio") {
										...
										if(!bitrateTimer) {
											...
											bitrateTimer = setInterval(function() {
												...
												let bitrate = helloworldtest.getBitrate();
									...
									$('#toggleaudio').click(
										function() {
											...
											helloworldtest.send({ message: { audio: audioenabled }});
										});
									$('#togglevideo').click(
										function() {
											...
											helloworldtest.send({ message: { video: videoenabled }});
										});
									...
									$('#bitrate a').click(function() {
										...
										helloworldtest.send({ message: { bitrate: bitrate }});
										return false;
									});
								},
...
function addSimulcastSvcButtons(temporal) {
	...
	$('#sl-0').removeClass('btn-primary btn-success').addClass('btn-primary')
		.unbind('click').click(function() {
			...
			if(simulcastStarted)
				helloworldtest.send({ message: { substream: 0 }});
			else
				helloworldtest.send({ message: { spatial_layer: 0 }});
		});
	$('#sl-1').removeClass('btn-primary btn-success').addClass('btn-primary')
		.unbind('click').click(function() {
			...
			if(simulcastStarted)
				helloworldtest.send({ message: { substream: 1 }});
			else
				helloworldtest.send({ message: { spatial_layer: 1 }});
		});
	$('#sl-2').removeClass('btn-primary btn-success').addClass('btn-primary')
		.unbind('click').click(function() {
			...
			if(!$('#sl-0').hasClass('btn-success'))
				$('#sl-0').removeClass('btn-primary btn-info').addClass('btn-primary');
			if(simulcastStarted)
				helloworldtest.send({ message: { substream: 2 }});
			else
				helloworldtest.send({ message: { spatial_layer: 2 }});
		});
	...
	$('#tl-0').removeClass('btn-primary btn-success').addClass('btn-primary')
		.unbind('click').click(function() {
			...
			if(simulcastStarted)
				helloworldtest.send({ message: { temporal: 0 }});
			else
				helloworldtest.send({ message: { temporal_layer: 0 }});
		});
	$('#tl-1').removeClass('btn-primary btn-success').addClass('btn-primary')
		.unbind('click').click(function() {
			...
			if(simulcastStarted)
				helloworldtest.send({ message: { temporal: 1 }});
			else
				helloworldtest.send({ message: { temporal_layer: 1 }});
		});
	$('#tl-2').removeClass('btn-primary btn-success').addClass('btn-primary')
		.unbind('click').click(function() {
			...
			if(!$('#tl-0').hasClass('btn-success'))
				$('#tl-0').removeClass('btn-primary btn-info').addClass('btn-primary');
			if(simulcastStarted)
				helloworldtest.send({ message: { temporal: 2 }});
			else
				helloworldtest.send({ message: { temporal_layer: 2 }});
		});
}
